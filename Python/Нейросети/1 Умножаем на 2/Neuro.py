...
while True:
    ...
    while prediction is None or prediction[0][0] != 2*(int_X_value): # *****НОВАЯ ПАРА ЗНАЧЕНИЙ +1*****
        epochs += 1
        model.fit(X_train, y_train, epochs=1, verbose=0)
        ...
        y_test = np.array([[2*(int_X_value)]], dtype=float) # *****НОВАЯ ПАРА ЗНАЧЕНИЙ +1*****
        loss = model.evaluate(x_test, y_test, verbose=0)
        loss_history.append(loss)

        # Проверяем предсказание для числа 8
        prediction = model.predict(x_test)
