...
# Пример данных для обучения (программные фрагменты на Python)
data = [
    "def hello_world():\n    print('Hello, world!')",
    "import numpy as np\nx = np.array([1, 2, 3])",
    "for i in range(10):\n    print(i)",
...
X = pad_sequences(X, maxlen=100)  # Увеличьте maxlen, если ваши фрагменты кода длиннее
...
def generate_code(input_string, max_length=100):
    ...
    for _ in range(max_length):
        ...
        input_sequence = np.append(input_sequence[0][1:], predicted_index)
        input_sequence = input_sequence.reshape(1, -1)
    
    return generated_code

# Пример использования
user_input = input("Введите начальный фрагмент кода на Python: ")
prediction = generate_code(user_input)
